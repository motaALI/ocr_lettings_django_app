...
class TestProfileTemplate(TestCase):
    ...
    def test_profile_details_template(self, mock_profile):
        ...
        response = self.client.get(
            reverse("profile", args=[profile_data.user.username])
        )
        ...
        self.assertContains(
            response,
            f'<h1 class="page-header-ui-title mb-3 display-6">{profile_data.user.username}</h1>',
        )
        self.assertContains(
            response,
            f"<p><strong>First name :</strong> {profile_data.user.first_name}</p>",
        )
        self.assertContains(
            response,
            f"<p><strong>Last name :</strong> {profile_data.user.last_name}</p>",
        )
        self.assertContains(
            response,
            f"<p><strong>Email :</strong>{profile_data.user.email}</p>",
            html=True,
        )
